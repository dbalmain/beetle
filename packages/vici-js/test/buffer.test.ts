...
import { Utf8Buffer } from "../src/buffer.js";
import { JsBuffer } from "../src/buffer-js.js";
import { advance, invertChange, shift } from "../src/index.js";

const implementations = [
  { name: "js string", Buffer: JsBuffer },
  { name: "utf8 pieces", Buffer: Utf8Buffer },
] as const;

for (const { name, Buffer } of implementations) {
  describe(`${name}: rows are counted by LF only`, () => {
  ...
  describe(`${name}: byte / point mapping`, () => {
  ...
  describe(`${name}: insert and delete update points`, () => {
  ...
  describe(`${name}: insert/delete invert`, () => {
    it("restores bytes and points", () => {
      ...
      expect(
        [0, 5, 6, buffer.lenBytes()].map((b) => buffer.byteToPoint(b)),
      ).toEqual(beforePoints);
    });
  ...
  describe(`${name}: many small inserts`, () => {
    it("stays correct after 600 one-char inserts", () => {
      const buffer = new Buffer();
      let expected = "";
      for (let i = 0; i < 600; i++) {
